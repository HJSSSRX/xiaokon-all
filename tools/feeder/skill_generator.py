...
class SkillGenerator:
    ...
    SKILL_DOMAINS = {
        ...
        "server": {
            ...
            "subcategories": [
                ...
                "database_forensics",
                "mail_server_forensics",
                "middleware_forensics",
            ]
        },
        "binary": {
            "name": "二进制/恶意代码分析",
    ...
    def generate_skill_from_source(data: dict, kb_dir: str) -> str:
        ...
        # 推断领域和子类别
        domain, subcategory = SkillGenerator._infer_domain_from_source(data)
        ...
        skill_entry = {
            ...
            "summary": SkillGenerator._generate_summary(text_content),
            "description": text_content[:1000] if text_content else "",
            "sources": [f"sources/articles/{domain}/{safe_name}"],
            "practice": [],
            "techniques": SkillGenerator._extract_techniques(text_content),
            "tools": SkillGenerator._extract_tools(text_content, code_blocks),
            "commands": SkillGenerator._extract_commands(code_blocks),
            "difficulty": SkillGenerator._infer_difficulty(text_content),
            "prerequisites": [],
            "related_skills": [],
            "tags": SkillGenerator._extract_tags(data),
        ...
        # 更新技能索引
        SkillGenerator._update_skill_index(kb_path, domain, safe_name, skill_entry)
    ...
    def generate_skill_from_practice(data: dict, kb_dir: str) -> str:
        ...
        # 推断领域和子类别
        domain, subcategory = SkillGenerator._infer_domain_from_practice(data)
    ...
    def merge_skills(kb_dir: str, skill_paths: list, output_name: str) -> str:
        ...
        skill_file = kb_path / "skills" / merged_skill["domain"] / f"{safe_name}.yaml"
    ...
    def _infer_domain_from_practice(data: dict) -> tuple:
        ...
        # 根据标签推断
        return SkillGenerator._infer_domain_from_source({"title": "", "text_content": text})
