...
class FeederOrganizer:
    ...
    def organize_article_to_kb(data: dict, kb_dir: str):
        ...
        # 推断领域分类
        domain = FeederOrganizer._infer_domain(data)
        ...
        source_entry = {
            ...
            "tags": FeederOrganizer._extract_tags(data),
            "related_practice": [],
            "content_preview": data.get("text_content", "")[:500],
        }
        
        # 保存到 sources/articles/{domain}/
        ...
        # 同时保存完整内容到 skills（融合层）
        FeederOrganizer._update_skills_index(kb_path, domain, safe_name, data)
        
        # 更新索引
        FeederOrganizer._update_sources_index(kb_path, domain, safe_name, source_entry)
        
        # 自动产出技能
        from .skill_generator import SkillGenerator
        SkillGenerator.generate_skill_from_source(data, kb_dir)
    ...
    @staticmethod
    def organize_solved_to_kb(data: dict, kb_dir: str):
        ...
        # 保存到 practice/solved/{competition}/
        ...
        # 更新索引
        FeederOrganizer._update_practice_index(kb_path, competition, question_id, practice_entry)
        
        # 自动产出技能
        from .skill_generator import SkillGenerator
        SkillGenerator.generate_skill_from_practice(data, kb_dir)
    ...
    def _update_skills_index(kb_path: Path, domain: str, name: str, data: dict):
        ...
        skill_entry = {
            ...
            "source": data.get("url", ""),
            "content": "\n".join(content_lines),
            "keywords": FeederOrganizer._extract_keywords_from_text(text_content[:500]),
        }
