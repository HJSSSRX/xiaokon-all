"""HTTP request handler for collaboration hub."""
...
class Handler(BaseHandler):
    """Collaboration Hub HTTP handler. Inherits _send/_err/_read_json/_split_path/do_OPTIONS from BaseHandler."""
    ...
    def do_GET(self):
        ...
        if path == "/heartbeat":
            ...
            for role, info in beats.items():
                ...
                if offline:
                    alert_list.append({
                        "role": role, "status": "offline", "age_seconds": age_seconds,
                        "last_heartbeat": last,
                        "message": f"角色 {role} 已离线超过 30 分钟",
                    })
                elif stale:
                    alert_list.append({
                        "role": role, "status": "stale", "age_seconds": age_seconds,
                        "last_heartbeat": last,
    ...
    def _dispatch_post(self, path, body):
        if path == "/log":
            ...
            if kind == "answer":
                ...
                forwarded = {
                    "category": category, "qid": body.get("qid", ""),
                    "question": body.get("question", ""), "answer": body.get("answer", ""),
                    "confidence": body.get("confidence", "medium"), "source_role": role,
                    "evidence": body.get("evidence", ""), "analysis": body.get("analysis", ""),
                    "evidence_path": body.get("evidence_path", []),
                }
                return self._dispatch_post("/answers", forwarded)
            if kind == "finding":
                forwarded = {
                    "from": role, "type": body.get("type", "evidence"),
                    "summary": body.get("summary", ""), "detail": body.get("detail", ""),
                    "related_to": body.get("related_to", []),
                }
                return self._dispatch_post("/findings", forwarded)
            if kind == "blocker":
                forwarded = {
                    "from": role, "blocker": body.get("blocker", ""),
                    "needs": body.get("needs", ""), "routed_to": body.get("routed_to", ""),
                }
                return self._dispatch_post("/session/blocker", forwarded)
            if kind == "question":
                forwarded = {
                    "from": role, "to": body.get("to", "main_designer"),
                    "question": body.get("question", ""), "context": body.get("context", ""),
                }
        ...
        if path == "/session/blocker":
            ...
            entry = {
                ...
                "time": now_str(), "from": body.get("from", ""),
                "blocker": body.get("blocker", ""),
        ...
        if path == "/needs":
            ...
            entry = {
                ...
                "time": now_str(), "from": role,
                "item": item, "purpose": body.get("purpose", ""),
                "candidate_locations": body.get("candidate_locations", []),
                "candidate_providers": body.get("candidate_providers", ["*"]),
                "blocking_qids": body.get("blocking_qids", []),
                "deadline_hours": body.get("deadline_hours"),
                "status": "open", "claimed_by": "",
                "fulfilled_by": "", "fulfilled_value": "", "fulfilled_at": "",
                "updated": now_str(),
            }
            needs.append(entry)
        ...
        if m:
            ...
            if existing and isinstance(existing, dict):
                try:
                    locked_at = datetime.datetime.strptime(existing.get("locked_at", ""), "%Y-%m-%d %H:%M:%S")
        ...
        if path == "/answer_locks/clean":
            ...
            save_yaml(fpath, locks)
            return self._send(200, {
                "status": "cleaned", "before": before,
                "after": after, "cleaned": cleaned,
            })
