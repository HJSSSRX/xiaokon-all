#!/usr/bin/env python3
"""
AutoForensicAI 赛事知识库检索工具

用法:
    python tools/comp_search.py --keywords 推广设计图 apk
    python tools/comp_search.py --question "服务器上怎么查 IP 字段"
    python tools/comp_search.py --category server
    python tools/comp_search.py --result incorrect    # 只看错题
    python tools/comp_search.py --tech rsa            # 找 rsa 相关技巧卡
    python tools/comp_search.py --all                 # 列出全部
...
def resolve_kb_root(cli_arg: str | None = None) -> Path | None:
    """Find the competition knowledge base directory.

    Search order (first hit wins):
      1. --kb-dir CLI arg
      2. $AUTOFORENSICAI_KB env var
      3. <repo>/knowledge/competitions/<赛事>/ (any subdir with problems/)
      4. <repo>/cases/*/knowledge_base
      5. <repo>/data/kb
    ...
    # knowledge/competitions/<赛事>/
    comps_dir = REPO_ROOT / "knowledge" / "competitions"
    if comps_dir.is_dir():
        for sub in comps_dir.iterdir():
            if sub.is_dir():
                candidates.append(sub)
    # cases/*/knowledge_base (legacy)
...
def fmt_technique(t: dict, score: int) -> str:
    ...
    lines = [
        f"  [{score} pts] \U0001f527 {name} ({cat})",
        f"      slug: {t.get('slug')}",
...
def fmt_retro(r: dict, score: int) -> str:
    role = r.get("role", "?")
    topic = r.get("topic", "?")
    return f"  [{score} pts] \U0001f4a1 {role} | {topic}"
...
def cmd_keywords(args):
    ...
    if p_results:
        print(f"\U0001f4cb 题目卡 ({len(p_results)}):")
        for s, p in p_results:
            print(fmt_problem(p, s))
            print()
    else:
        print("\U0001f4cb 题目卡: 无命中")

    if t_results:
        print(f"\n\U0001f527 技巧卡 ({len(t_results)}):")
    ...
    if r_results:
        print(f"\n\U0001f4a1 经验卡 ({len(r_results)}):")
        for s, r in r_results:
            print(fmt_retro(r, s))
    else:
        print("\n\U0001f4a1 经验卡: 无 (远程机暂未上传)")
...
def cmd_all(args):
    ...
    retros = load_retrospectives()
    print(f"\n=== 知识库总览 ===\n")
    print(f"\U0001f4cb 题目卡: {len(problems)}")
    by_cat = {}
    ...
    print(f"\n\U0001f527 技巧卡: {len(techs)}")
    for t in sorted(techs, key=lambda x: x.get("slug", "")):
        print(f"   - {t.get('slug', '?')}: {t.get('name', '?')}")
    print(f"\n\U0001f4a1 经验卡: {len(retros)}")
    for r in retros:
        print(f"   - {r.get('role')}: {r.get('topic')}")


def main():
    p = argparse.ArgumentParser(description="AutoForensicAI 赛事知识库检索")
    ...
    p_cat.add_argument("category")  # dynamic — no hardcoded choices
    ...
    p.add_argument("--category", help="同 category 子命令（不再限制取值）")
    ...
    if kb is None:
        print("[comp_search] 找不到 KB 根目录. 候选搜索路径全部失败.", file=sys.stderr)
